...
export default function DetailReport() {
  ...
  const cancelUploadImage = async (itemIndex) => {
    ...
    console.log(reportOfficer);
    const { data } = await axios.delete(`http://localhost:5500/delete/image/${imageName}`, {
      headers: {
        Authorization: `Bearer ${auth.user ? auth.token : ''}`,
      },
    });
              ...
              <div className=''>
                ...
                <div className='col-span-1'>
                  ...
                  <div className='text-center mt:4 md:mt-8 w-full'>
                    <button className='bg-blue-600 mt-4 float-right py-2 px-6 rounded-lg focus:outline-none transition-all ease-out text-white hover:bg-blue-700 focus:bg-blue-900' onClick={(e) => handleSubmit(e)}>
                      Kirim
                    </button>
                  </div>
                </div>
