      ...
      <ul className='w-full space-y-2 font-medium'>
        ...
        {userMenus &&
          userMenus[auth.user.role].map((menu, index) => (
            <li key={index}>
              <NavLink to={menu.route} onClick={closeSidebar} className='w-full group flex items-center p-2 text-gray-400 hover:bg-[#1E282C] hover:border-l-2 hover:border-primary-500 transition ease-in duration-75'>
